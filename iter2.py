...
# options = {'probesize': '32', 'fflags': 'nobuffer'}
# container = av.open(udp_addr, options=options)
...
try:
    ...
    # transform the byte read into a numpy array
        ...
        image = image.reshape((720, 960, 3)) # Notice how height is specified first and the width is specified second
        ...
        if rect is not None:
            ...
            print(int(pan_update), int(tilt_update))
            drone.send_rc_control(int(pan_update) // 3, int(forward_tilt), int(tilt_update) // 2, 0)
        pipe.stdout.flush()
